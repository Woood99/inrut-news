...
import { tooltipSecondary, tooltipMain } from "./components/tooltips";
...
`,
    ...
    document
        ...
        .forEach((filter) => filterCounter(filter));
    document
        .querySelectorAll(".filter")
        .forEach((filter) => filterClear(filter));
    document
        .querySelectorAll(".quantity-selection")
        .forEach((container) => quantitySelection(container));

    const inputsMaskPhone = document.querySelectorAll(".input-phone-mask");
    const inputsMaskSeriesNumber = document.querySelectorAll(
        ".input-series-number-mask"
    );
    const inputsMaskDepartCode = document.querySelectorAll(
        ".input-depart-code-mask"
    );
    ...
    const inputsCardValidityMask = document.querySelectorAll(
        ".input-card-validity-mask"
    );
    ...
    dropdownItems(
        ".object-characteristics__container",
    ...
    document.addEventListener("click", (e) => {
        ...
        const metroInfoActive = document.querySelectorAll(
            ".metro-info__other._active"
        );
        if (searchSelectOne.length > 0) {
            const currentContainer = target.closest(".search-select-one");
            if (
                (currentContainer &&
                    !currentContainer.classList.contains("_active")) ||
                !currentContainer
            ) {
                searchSelectOne.forEach((container) =>
        ...
        if (selectThird.length > 0) {
            ...
            if (
                (currentContainer &&
                    !currentContainer.classList.contains("_show")) ||
                !currentContainer
            ) {
                selectThird.forEach((container) =>
    ...
    if (employeeRole && selects.length > 0) {
        ...
        function body(values) {
            const setItems = Array.from(selects).filter((item) =>
                values.set.includes(item.dataset.employeeSelectName)
            );
            const hiddenItems = Array.from(selects).filter((item) =>
                values.hidden.includes(item.dataset.employeeSelectName)
            );
    ...
    if (createPresents.length > 0) {
        createPresents.forEach((createPresent) => {
            const checkboxes = createPresent.querySelector(
                ".place-present__checkboxes"
            );
            const summBlock = createPresent.querySelector(
                ".place-present__summ"
            );
            const quantityBlock = createPresent.querySelector(
                ".place-present__quantity"
            );
            checkboxes.addEventListener("change", (e) => {
                const target = e.target;
                const summ = target.closest(".place-present-checkbox-summ");
                const quantity = target.closest(
                    ".place-present-checkbox-quantity"
                );
                if (summ) {
    ...
    if (createPresentsField) {
        const input = createPresentsField.querySelector(
            "[data-create-present-input]"
        );
        const apartments = createPresentsField.querySelector(
            "[data-create-present-apartments]"
        );
        const select = createPresentsField.querySelector(
            "[data-create-present-select]"
        );
        ...
        apartments.addEventListener("change", (e) => {
            const isActive = Array.from(apartmentsItems).find(
